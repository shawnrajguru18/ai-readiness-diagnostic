...
# 
# Usage: bash deploy/aws/destroy_terraform.sh [--auto-approve] [--skip-backup]
#        (runnable from any directory — paths are resolved from the script location)
#
# Options:
...
#
# SCOPE — read this before running. This directory holds two teardown scripts that
# delete DIFFERENT resource sets, because the project has two provisioning paths:
#
#   03-teardown.sh        undoes 01-bootstrap.sh + 02-deploy.sh (see config.sh):
#                         ai-readiness-cluster, ai-readiness-sessions
#   destroy_terraform.sh  undoes terraform/ (app_name = ai-readiness-diagnostic):
#                         ai-readiness-diagnostic-cluster, ...-sessions
#
# Running the wrong one succeeds while deleting nothing. Confirm which path created
# the live resources first — deploy/aws/pre_destroy_checklist.md step 2.3.
#
################################################################################
...
main() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Run post-destroy validation: bash $SCRIPT_DIR/post_destroy_validation.sh"
